...
* All the pipes are **safe at runtime**. I.E: when a source is disconnected or temporarily unavailable, the main loop can continue without necessarily having to check errors (they can be checked, anyway, by polling the status of each node: see [this example](https://github.com/paolo-pr/laav/blob/master/examples/AudioVideoExample_2.cpp))
